...
main = do
  ...
  verifyDefault (Info, vconfQuiet) (\x y -> assert (adderDelayRapidBit @256 x y === (delay 0 (x + y))) "adderDelayRapidBit === + (256)")
  verifyDefault (Info, vconfQuiet) (\x y -> assert (adderDelayRapidBit @512 x y === (delay 0 (x + y))) "adderDelayRapidBit === + (512)")
  verifyDefault (Info, vconfQuiet) (\x y -> assert (adderDelayRapidBit @100 x y === (delay 0 (x + y))) "adderDelayRapidBit === + (100)")
  verifyDefault (Info, vconfQuiet) (\x y -> assert (adderDelayRapidBit @2 x y === (delay 0 (x + y))) "adderDelayRapidBit === + (2)")
  verifyDefault (Info, vconfQuiet) (\x y -> assert (adderDelayRapidBit @3 x y === (delay 0 (x + y))) "adderDelayRapidBit === + (3, too hard)")
  ...
  verifyDefault (Info, vconfQuiet) (\x y -> assert ((delay 0 (adderSeqUnrollBit @2 x y)) === (delay 0 (x + y))) "asu: inner delays (too hard)")
  ...
  verifyDefault (Info, vconfQuiet) (\x y -> assert ((delay 0 $ adderSeqUnrollBit @2 x y) === adderDelaySeqTVecBit @2 x y) "d.asu === astv (too hard)")
  verifyDefault (Info, vconfQuiet) (\x y -> assert ((delay 0 $ adderSeqUnrollBit @2 x y) === adderDelaySeqTVecBit @2 x y) "d.asu === astv (too hard)")
  verifyDefault (Info, vconfQuiet) (\x -> assert ((V.head $ f x) === 1) "unroll . slowdown === map // head")
  verifyDefault (Info, vconfQuiet) (\x -> assert ((V.last $ f x) === 1) "unroll . slowdown === map // last")
  verifyDefault (Info, vconfQuiet) (\x -> assert ((V.head $ f x, V.last $ f x) === (1, 1)) "unroll . slowdown === map // head and last (too hard -> few seconds)")
  where
