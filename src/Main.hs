...
main = do
  ...
  --verifyLiveIncremental cnf (\x y -> assert (((test_comb x y) === (test_seq x y))) "test_comb === test_seq")
  --verifyLiveIncremental cnf (\x y -> assert (((test_seq x y) === (test_seq x y))) "test_seq === test_seq")
  --verifyWith cnfEasy (\x y -> assert (((test_seq x y) === (test_seq x y))) "test_seq === test_seq")
  --verifyWith cnfEasy (\x y -> assert ((((delay 0 *** delay 0) $ test_comb x y) === ((delay 0 *** delay 0) $ test_seq x y))) "delay . test_comb === delay . test_seq")
  --verifyLiveFixed (Verbose, vconfDefault, FixedConf { depth = 130 }) (\x y -> assert ((((delay 0 *** delay 0) $ test_comb x y) === ((delay 0 *** delay 0) $ test_seq x y))) "delay . test_comb === delay . test_seq")
  --verifyLiveIncremental cnf (\x y -> assert ((((delay 0 *** delay 0) $ test_comb x y) === ((delay 0 *** delay 0) $ test_seq x y))) "delay . test_comb === delay . test_seq")
  --verifyOfflineFixed (Verbose, vconfDefault, FixedConf { depth=4 }) (assert (((delay 0 (test6 2 ())) === (delay 0 (test6 2 ())))) "test6: inner delays") "test6" smtDir
  --verifyLiveFixed (Verbose, vconfDefault, FixedConf { depth=4 }) (assert (((delay 0 (test6 2 ())) === (delay 0 (test6 2 ())))) "test6: inner delays")
  -- long? verifyLiveIncremental cnf (assert (((delay 0 (test6 2 ())) === (delay 0 (test6 2 ())))) "test6: inner delays")
  -- long? verifyLiveIncremental cnf (\x y -> assert (((delay 0 (test4 x y)) === (delay 0 (x + y)))) "test4: inner delays")
  --verifyLiveIncremental cnf (\x -> assert ((notId x) === x) "notId === id")
  --verifyLiveIncremental cnf (\x y -> assert (adder @16 x y === x + y) "adder === +")
  --verifyLiveIncremental cnf (\x y -> assert (adderSeqUnrollBit @16 x y === x + y) "adderSeqUnrollBit === +")
  --verifyLiveIncremental cnf (\x y -> assert (adderDelaySeqTVecBit @2 x y === (delay 0 (x + y))) "adderDelaySeqTVecBit === delay . +")
  --verifyLiveIncremental cnf (\x y -> assert (adderBatchUnrollBit @16 x y === x + y) "adderBatchUnrollBit === +")
  ---- long? verifyLiveIncremental cnf (\x y -> assert (adderDelayRapidBit @2 x y === (delay 0 (x + y))) "adderDelayRapidBit === + (too hard)")
  --verifyLiveIncremental cnf (\x y -> assert ((test2 x y) === (x + y)) "test2: no delays")
  --verifyLiveIncremental cnf (\x y -> assert (delay 1 ((test2 x y) === (x + y))) "test2: outer delay")
  --verifyLiveIncremental cnf (\x y -> assert (((delay 0 (test2 x y)) === (delay 0 (x + y)))) "test2: inner delays")
  --verifyLiveIncremental cnf (\x y -> assert ((adderSeqUnrollBit @2 x y) === (x + y)) "asu: no delays")
  --verifyLiveIncremental cnf (\x y -> assert (delay 1 ((adderSeqUnrollBit @2 x y) === (x + y))) "asu: outer delay")
  ---- long? verifyLiveIncremental cnf (\x y -> assert ((delay 0 (adderSeqUnrollBit @2 x y)) === (delay 0 (x + y))) "asu: inner delays (too hard)")
  --verifyLiveIncremental cnf (\x y -> assert ((adderSeqUnrollBit @2 x y) === (x + y)) "asu === +")
  --verifyLiveIncremental cnf (\x y -> assert ((delay 0 $ x + y) === adderDelaySeqTVecBit @2 x y) "d.+ === astv")
  verifyLiveFixed (Verbose, vconfDefault, FixedConf { depth = 17 }) (\x y -> assert ((delay 0 $ adderSeqUnrollBit @2 x y) === adderDelaySeqTVecBit @2 x y) "d.asu === astv (too hard)")
  verifyLiveIncremental cnf (\x y -> assert ((delay 0 $ adderSeqUnrollBit @2 x y) === adderDelaySeqTVecBit @2 x y) "d.asu === astv (too hard)")
  verifyLiveIncremental cnf (\x -> assert ((V.head $ f x) === 1) "unroll . slowdown === map // head")
  verifyLiveIncremental cnf (\x -> assert ((V.last $ f x) === 1) "unroll . slowdown === map // last")
  verifyLiveIncremental cnf (\x -> assert ((V.head $ f x, V.last $ f x) === (1, 1)) "unroll . slowdown === map // head and last (too hard -> few seconds)")
  where
